    ...
    <>
      ...
      <aside className={`fixed lg:sticky top-0 left-0 w-64 md:w-72 bg-secondary-950 text-white h-screen flex flex-col shadow-2xl z-[70] border-r border-white/5 transition-transform duration-300 transform flex-shrink-0 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 md:p-8 border-b border-white/5 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[8px] md:text-[10px] text-primary-500 font-black uppercase tracking-widest mt-1">Admin</span>
            </div>
          </Link>
        ...
        <nav className="flex-grow py-6 md:py-8 px-4 md:px-5 space-y-1.5 md:space-y-2 overflow-y-auto custom-scrollbar">
          <p className="px-4 text-[9px] md:text-[10px] font-black text-secondary-500 uppercase tracking-[0.3em] mb-4">Main Navigation</p>
          {menuItems.map((item) => {
